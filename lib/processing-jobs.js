...
function createProcessingJobStore(db) {
  function audit(job, eventType, details = {}) {
    db.prepare(`
      INSERT INTO audit_events
        (actor_user_id, event_type, entity_type, entity_id, occurred_at, details_json)
      VALUES (?, ?, 'resource', ?, ?, ?)
    `).run(
      job.actor_user_id, eventType, job.resource_id, nowIso(),
      JSON.stringify({ jobId: job.id, attempts: job.attempts, maxAttempts: job.max_attempts, ...details })
    );
  }
  ...
  function fail(id, error) {
    ...
    const timestamp = nowIso();
    const conflict = error && error.code === "PROCESSING_VERSION_CONFLICT";
    db.transaction(() => {
      db.prepare(`
      ...
      audit(job, conflict ? "processing_version_conflict" : "processing_failed", {
        outcome: retry ? "retry_queued" : "failed"
      });
    })();
