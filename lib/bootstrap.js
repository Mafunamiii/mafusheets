...
async function main() {
  ...
  try {
    ...
    if (command === "change-role") {
      const user = users.setRole(argumentValue("--id"), argumentValue("--role"));
    ...
    throw new Error(
      "Usage: create-user --login LOGIN --display-name NAME --role admin|member | disable-user --id ID | enable-user --id ID | reset-password --id ID | change-role --id ID --role admin|member | list-users"
    );
  } finally {
    db.close();
  }
