...
function createResourceStore(db) {
  ...
  function createBatch(resources, moves, actorId, hooks = {}) {
    ...
    try {
      ...
      transactionImmediate(() => {
        ...
        `).run(
          operationId, actorId,
          json({ resourceIds: resources.map((item) => item.id) }),
          pendingTimestamp, pendingTimestamp
        );
      });
      hooks.afterPendingOperation?.(operationId);
      ...
      const existing = db.prepare("SELECT id FROM pending_operations WHERE id=?").get(operationId);
      if (existing) {
        transactionImmediate(() => {
          db.prepare(`
            UPDATE pending_operations SET status='failed', error_text=?, updated_at=? WHERE id=?
          `).run(String(error && error.message || error).slice(0, 1000), nowIso(), operationId);
          audit(actorId, "upload_failed", "catalog", null, {
            operationId, reason: String(error && error.message || error).slice(0, 240)
          });
        });
      } else {
        recordFailure({
  ...
  function markProcessingFailed(id, expectedUpdatedAt, actorId, reason) {
    return transactionImmediate(() => {
      const timestamp = nowIso();
      const changed = db.prepare(`
        UPDATE resources SET search_status='failed', updated_by=?, updated_at=?
        WHERE id=? AND deleted_at IS NULL AND updated_at=?
      `).run(actorId, timestamp, id, expectedUpdatedAt).changes;
      if (!changed) return false;
      db.prepare(`
        UPDATE pending_operations SET status='failed', error_text=?, updated_at=?
        WHERE operation_type='thumbnail' AND resource_id=?
          AND status IN ('pending', 'failed')
      `).run(String(reason || "processing failed").slice(0, 240), timestamp, id);
      audit(actorId, "processing_resource_failed", "resource", id, {
        reason: String(reason || "processing failed").replace(/[^\w .:-]/g, "").slice(0, 240)
      });
      return true;
    });
  }
  ...
  function updateSearchIndex(id, expectedUpdatedAt, result, actorId) {
    return transactionImmediate(() => {
      const timestamp = nowIso();
      const changed = db.prepare(`
        UPDATE resources SET search_text=?, search_pages_json=?, search_status=?, indexed_at=?,
          updated_by=?, updated_at=?
        WHERE id=? AND deleted_at IS NULL AND updated_at=?
      `).run(
        result.searchText, JSON.stringify(result.searchPages || []), result.searchStatus,
        timestamp, actorId, timestamp,
