...
const SCHEMA_V7 = `
ALTER TABLE resources ADD COLUMN search_pages_json TEXT NOT NULL DEFAULT '[]'
  CHECK (json_valid(search_pages_json));
`;
...
function applySchemaMigrations(db) {
  ...
  if (!applied.has(7)) {
    db.transaction(() => {
      ...
      db.prepare(
        "INSERT INTO schema_migrations (version, name, applied_at) VALUES (?, ?, ?)"
      ).run(7, "page-aware document search", nowIso());
    })();
  }
...
function mapResourceRow(row, annotations = []) {
  ...
  return {
    ...
    searchPages: JSON.parse(row.search_pages_json || "[]"),
      ...
      for (const resource of resources) {
        ...
        if (!current) {
          db.prepare(`
            INSERT INTO resources
              (id, title, artist, musical_key, capo, bpm, notes, tags_json, sheet_kind,
               search_text, search_pages_json, search_status, indexed_at, uploaded_by, updated_by, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).run(
            ...
            resource.searchText || "", JSON.stringify(resource.searchPages || []),
          ...
          db.prepare(`
            UPDATE resources SET title=?, artist=?, musical_key=?, capo=?, bpm=?, notes=?,
              tags_json=?, sheet_kind=?, search_text=?, search_pages_json=?, search_status=?, indexed_at=?,
          ...
          `).run(
            ...
            JSON.stringify(resource.searchPages || []),
            resource.searchStatus || "empty", resource.indexedAt || null,
